...
$$\mathbf{z}_{\text{pooled}} = \text{GridSample}\Big( \tilde{\mathbf{z}}_{\text{VGGT}}, \, \text{target\_tokens} = S_{\text{VLM}}, \, \text{mode} = \text{"bilinear"} \Big)$$
...
$$\mathcal{L}_{\text{WM}}^{\text{sem}} = \frac{1}{T \cdot S \cdot D} \sum_{t=1}^T \sum_{s=1}^S \Big\| \hat{\mathbf{s}}_{t, s}^{\text{sem}} - \text{stop\_grad}(\mathbf{s}_{t, s}^{\text{V-JEPA2}}) \Big\|_1$$
...
$$\mathcal{L}_{\text{WM}}^{\text{geo}} = \frac{1}{T \cdot N_p} \sum_{t=1}^T \sum_{p=1}^{N_p} \text{SmoothL1}\Big( \Delta \hat{\mathbf{p}}_{t, p}, \, \text{stop\_grad}(\Delta \mathbf{p}_{t, p}^{\text{canon}}) \Big)$$
