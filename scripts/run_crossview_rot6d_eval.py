#!/usr/bin/env python3
"""
Geo-JEPA: Benchmark Evaluator for Cross-View Attention + 6D Continuous Rotation ACT Policy.

Features:
1. Spatial 2D Grid Positional Encodings.
2. Bidirectional Cross-View Attention (AgentView ◄► Wrist).
3. 6D Continuous Rotation Matrix Conversion (Zhou et al., CVPR 2019).
4. Temporal Action Ensembling.
...
def evaluate_crossview_rot6d(suite_name: str = "libero_spatial", num_seeds: int = 10, device: str = "cuda"):
    out_dir = Path("/media/kavinder/hdd2/geo_jepa_eval_results/crossview_rot6d_eval")
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 85)
    print(f" GEO-JEPA: CROSS-VIEW ATTENTION + 6D ROTATION EVALUATION ({suite_name.upper()})")
    ...
    for task_idx in range(num_tasks):
        ...
        print(f"  --> CrossView Rot6D Success Rate: {sr:.1f}% ({succ_count}/{num_seeds})")
    ...
    report_path = out_dir / f"crossview_rot6d_{suite_name}_report.json"
    with open(report_path, "w") as f:
        json.dump(report, f, indent=2)

    print("\n" + "=" * 85)
    print(f" EVALUATION COMPLETE FOR {suite_name.upper()}!")
...
if __name__ == "__main__":
    ...
    evaluate_crossview_rot6d(suite_name="libero_spatial", num_seeds=10, device=device)
